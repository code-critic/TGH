...
void make_dataset(ostream &out_stream, uint n_vtx, uint n_edge_per_vtx, uint n_query) {
    ...
    for(uint u=0; u<n_vtx; u++) {
        for(uint i_ngh=0; i_ngh<n_edge_per_vtx; i_ngh++) {
            ...
            uint v = (u + rand()%(n_vtx-1))%n_vtx;           
            // every edge is stored only once (at bigger vtx)
            double prob = erf(4*runiform());    
            if (u>v) {
                // force sorted row
                auto &row = edges[u];
                Edge e = {v,prob};                
                auto it = upper_bound( row.begin(), row.end(), e, item, [](Edge a, Edge b){return a.target < b.target;} ),
                // remove duplicity
                if (it->target == v) {
                    i_ngh--; continue;
                }
    ...
    for(uint u=0;u<n_vtx; u++) {
        ...
        coords[u].x = runiform() * sqrt(n_vtx+10);
        coords[u].y = runiform() * sqrt(n_vtx+10);
        coords[u].power = runiform() + 0.5;
        for(uint i_ngh=0; i_ngh < edges[u].size(); i_ngh++) {
            uint v = edges[u][i_ngh].target;
            if (v>=u) cerr << "Wrong orientation: " << u << " , " << v <<endl;
            double dist = abs(coords[u].x - coords[v].x) + abs(coords[u].y - coords[v].y);
            if (dist < 0.01) { // guarantee prob < 1-1e-5
            ...
            if ( recieve_pst > 1-1e-5)
                cerr << "Broken prob. bound. 1-1e-5" << endl;
