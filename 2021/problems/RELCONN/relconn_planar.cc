...
void make_dataset(ostream &out_stream, uint n_vtx, uint n_edge, uint n_query) {
    
    vector<Coord> coords(n_vtx);
    for(Coord &item: coords) {
        // average density one vtx per 1km2
        item.x = runiform() * sqrt(n_vtx+10);
        item.y = runiform() * sqrt(n_vtx+10);
        item.power = runiform() + 0.5;
    }
    
    vector< vector<Edge> > edges(n_vtx);
    uint i_edg =0;
    for(uint u=0; u<n_vtx; u++) {
        uint n_ngh=n_edge - i_edg;
        if (u<n_vtx-1) n_ngh= n_ngh/(n_vtx - u) * (1.0 + 0.2*runiform() );
        for(uint v=0; v<n_vtx && n_ngh>0; v++) {
            if (u==v) continue;
            if (runiform() > n_ngh / double(n_vtx-1) ) continue;                 
            
            double dist = abs(coords[u].x - coords[v].x) + abs(coords[u].y - coords[v].y);
            double power = coords[u].power * coords[v].power;
            // average range 5km
            double rel_dist = dist / 5;
            double signal_gain = power / rel_dist / rel_dist;
            double decibels = log(signal_gain)+(2*runiform()-1);
            double recieve_pst = (1+erf(decibels))/2;
            
            //cout << dist << " " << signal_gain << " " << 
            //        decibels << " " << recieve_pst  << endl;
            if (recieve_pst == 1) continue;
            if (recieve_pst < 1e-2) continue;
    
            //cerr << "u: " << u << " v: " << v << " p: " << recieve_pst << endl;
            Edge e;
            e.target= v; e.prob =recieve_pst;
            edges[u].push_back(e);
            e.target= u; e.prob =recieve_pst;
            edges[v].push_back(e);
            i_edg ++;
            n_ngh --;
        }           
    ...
    // remove duplicit edges
    uint new_n_edges=0;
    for(uint i_row=0; i_row<edges.size(); i_row++) {
        auto &row=edges[i_row];
        sort(row.begin(), row.end(), [](Edge a, Edge b){return a.target < b.target;});        
        auto first = row.begin();
        auto result = first;
        //cout << "row: " << i_row << endl;
        // skip targets smaller then i_row
        while (++first != row.end() && first->target < i_row) { 
            //cout << "    " << first->target << " " << result->target << endl;
            if (result->target == first->target) {
                // average duplicate edges
                result->prob = (result->prob + first->prob)/2;
            } else if (++result != first) {
                // move others
                *result = std::move(*first);
            }    
        }
        if (result->target < i_row) ++result;
        
        row.erase(result, row.end());
        //cout << "row size: " << row.size() << endl;
        new_n_edges+=row.size();
    }
    
    // graph output
    stringstream graph_out;
    graph_out << n_vtx << " " << new_n_edges << endl;
    for(uint i_row=0; i_row<edges.size(); i_row++) {        
...
void make_data(ostream &out, unsigned int problem_size) {
  ...
  uint n_edge = 10*n_vtx;
  uint n_query = n_vtx / 10 + 50;

  stringstream input_data;
  make_dataset(input_data, n_vtx, n_edge, n_query);  
